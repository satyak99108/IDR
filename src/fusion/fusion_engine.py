...
class GNSSINSFusionEngine:
    ...
    def __init__(
        ...
        self._recovery_start_s: Optional[float] = None
    ...
    def step(
        ...
        has_gnss_fix = (
            not is_outage
            and gnss_lat is not None
            and gnss_lon is not None
            and not (np.isnan(gnss_lat) or np.isnan(gnss_lon))
        )

        if is_outage or not has_gnss_fix:
            self.mode = NavigationMode.DEAD_RECKONING
            self._outage_active = True
            self._recovery_start_s = None
        else:
            if self._outage_active:
                # GNSS just returned! Enter recovery mode
                self.mode = NavigationMode.RECOVERY
                self._outage_active = False
                self._recovery_start_s = timestamp_s
            elif self._recovery_start_s is not None:
                if (timestamp_s - self._recovery_start_s) > self.recovery_window_s:
                    self.mode = NavigationMode.GNSS_INS
                    self._recovery_start_s = None
                else:
                    self.mode = NavigationMode.RECOVERY
        ...
        if has_gnss_fix:
            ...
            self.ekf.update_gnss_pv(
                ...
                v_n_meas=v_n,
                v_e_meas=v_e,
                pos_std=3.0 if self.mode == NavigationMode.GNSS_INS else 5.0,
                vel_std=0.3,
            )
        ...
        # Confidence: inverse of covariance trace scaled to 0-100%
        trace = self.ekf.state_uncertainty_trace
        conf = float(np.clip(100.0 / (1.0 + 0.05 * trace), 10.0, 99.0))
